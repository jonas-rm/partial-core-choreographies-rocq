...
(** ** Properties of [XMerge] and related functions
  We start with some tactics. The second one should be merged with eq_dec_elim.
*)
...
(** We start with properties of [Xmerge] that are needed for some later proofs.
...
Lemma collapse_merge : forall (B B':XBehaviour Sig),
  collapse B = XUndefined -> collapse (B [[V]] B') = XUndefined.
Proof.
...
+ not_XUndef B HB'. not_XUndef B' HB''.
  simpl; rewrite HB; auto.
+ case_eq_dec a a0 Haa0.
  not_XUndef (B [[V]] B'1) HB'. not_XUndef B'2 HB''.
  simpl; rewrite IHB; auto.
+ not_XUndef B HB'.
  simpl; rewrite HB; auto.
+ not_XUndef B' HB'. not_XUndef B HB''.
  ...
  - generalize (IHB1 B'1 H); clear IHB1 IHB2.
    ...
    * revert H0. opt_elim o p; opt_elim o0 p; intros.
      ...
      2,3: elim (XUndefined_dec _ (collapse b1)); intros;
        [ case (B2 [[V]] B'2); simpl; try rewrite a0; auto
        | rewrite Xmatch_elim in H2; auto; inversion H2 ].
      collapse_check b1 b2 H2.
      inversion_clear H3.
      case (B2 [[V]] B'2); simpl; try rewrite H4; auto.
      case (B2 [[V]] B'2); simpl; case (collapse b1); try rewrite H4; auto.
    * collapse_check x x0 H0. clear H0.
      inversion_clear H1.
      case (B2 [[V]] B'2); simpl; try rewrite H0; auto.
      case (B2 [[V]] B'2); simpl; case (collapse x); try rewrite H0; auto.
  - generalize (IHB2 B'2 H); clear IHB1 IHB2.
    case (B2 [[V]] B'2); simpl; intros; auto.
    1,7: inversion H0.
    6: case (B1 [[V]] B'1); auto.
    1,2,3: elim (XUndefined_dec _ (collapse x)); intros;
      [ case (B1 [[V]] B'1); simpl; try rewrite H0; auto;
    ...
    * revert H0. opt_elim o p; opt_elim o0 p; intros.
      4: inversion H2.
      2,3: elim (XUndefined_dec _ (collapse b1)); intros;
      [ case (B1 [[V]] B'1); simpl; try rewrite a0; auto;
      ...
      inversion_clear H3.
      case (B1 [[V]] B'1); simpl; try rewrite H2; auto;
        intros; try case o1, o2; try induction p; try induction p0;
          try case (collapse x); try case (collapse x0);
...
Lemma Xmerge_inv_inject : forall (B1 B2:XBehaviour Sig) B,
  B1 [[V]] B2 = inject B -> exists B', B1 = inject B'.
Proof.
...
+ eq_dec_inj_elim b b0 Hbb0 H. clear Hbb0.
  clear IHB2_1 IHB2_2.
  elim (collapse_char' _ (B1_1 [[V]] B2_1)); intros.
  elim (collapse_char' _ (B1_2 [[V]] B2_2)); intros.
  ...
  all: assert (collapse (XCond b (B1_1 [[V]] B2_1) (B1_2 [[V]] B2_2)) = XUndefined).
  1,3: simpl; rewrite b1; auto.
  1: case collapse; auto.
  clear a. all: clear b1.
  - assert (inject B = collapse (XCond b (B1_1 [[V]] B2_1) (B1_2 [[V]] B2_2))).
...
Lemma Xmerge_inv_inject' : forall (B1 B2:XBehaviour Sig) B,
  B1 [[V]] B2 = inject B -> exists B', B2 = inject B'.
Proof.
intros.
...
Lemma Xmerge_inv : forall (B1 B2:XBehaviour Sig) B,
  B1 [[V]] B2 = inject B -> exists B'1 B'2,
    B1 = inject B'1 /\ B2 = inject B'2 /\ B'1 [V] B'2 = inject B.
Proof.
intros.
...
(** ** Branching order on extended behaviours *)
...
Lemma Xmerge_is_lub : forall B B1 B2,
  B [[>>]] B1 -> B [[>>]] B2 -> B [[>>]] B1 [[V]] B2.
Proof.
intros.
destroy H; destroy H0.
...
Lemma Xmore_branches_merge : forall B1 B2,
  B1 [[>>]] B2 <-> (collapse B1 <> XUndefined /\ B1 [[V]] B2 = B1).
Proof.
...
Lemma Xmerge_is_larger : forall B1 B2,
  collapse (B1 [[V]] B2) <> XUndefined -> B1 [[V]] B2 [[>>]] B1.
Proof.
intros.
...
Lemma Xmerge_is_larger' : forall B1 B2,
  collapse (B1 [[V]] B2) <> XUndefined -> B1 [[V]] B2 [[>>]] B2.
Proof.
intros B1 B2.
rewrite Xmerge_comm.
