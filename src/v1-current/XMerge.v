...
+ case_eq_dec e e0 Hee0. case_eq_dec a a0 Haa0.
  simpl. not_XUndef (Xmerge B B') HX.
...
+ case_eq_dec a a0 Haa0.
  not_XUndef (Xmerge B B') HX.
  simpl; rewrite IHB; auto.
+ not_XUndef B HB'. not_XUndef B' HB''.
  simpl; rewrite HB; auto.
+ case_eq_dec a a0 Haa0.
  not_XUndef (Xmerge B B'1) HB'. not_XUndef B'2 HB''.
  simpl; rewrite IHB; auto.
+ not_XUndef B HB'.
  simpl; rewrite HB; auto.
+ not_XUndef B' HB'. not_XUndef B HB''.
...
+ not_XUndef B'1 HB'.
  case_eq_dec a a' Haa'.
  not_XUndef (Xmerge B B'2) HB''.
  simpl. rewrite IHB; auto.
  case (collapse B'1); auto.
+ not_XUndef B1 HB'. not_XUndef B2 HB''.
+ case_eq_dec a a0 Haa0.
  not_XUndef (Xmerge B1 B') HB'. not_XUndef B2 HB''.
  collapse_check B1 B2 H. inversion_clear H0; simpl.
  rewrite IHB1; auto.
  rewrite H1; auto. case (collapse (Xmerge B1 B')); auto.
+ not_XUndef B1 HB'. case_eq_dec a' a0 Ha'a0.
  not_XUndef (Xmerge B2 B') HB''.
  collapse_check B1 B2 H. inversion_clear H0; simpl.
  rewrite H1; auto. rewrite IHB2; auto.
  case (collapse B1); auto.
+ case_eq_dec a a0 Haa0. not_XUndef (Xmerge B1 B'1) HB'.
  case_eq_dec a' a'0 Ha'a'0. not_XUndef (Xmerge B2 B'2) HB''.
  collapse_check B1 B2 H. inversion_clear H0; simpl.
  rewrite IHB1; auto.
  rewrite IHB2; auto. case (collapse (Xmerge B1 B'1)); auto.
+ case_eq_dec b b0 Hbb0.
  ...
  - generalize (IHB1 B'1 H); clear IHB1 IHB2.
    ...
    * revert H0. opt_elim o p; opt_elim o0 p; intros.
      ...
      inversion_clear H3.
      case (Xmerge B2 B'2); simpl; try rewrite H4; auto.
      case (Xmerge B2 B'2); simpl; case (collapse b1); try rewrite H4; auto.
  ...
  - generalize (IHB2 B'2 H); clear IHB1 IHB2.
    ...
    * revert H0. opt_elim o p; opt_elim o0 p; intros.
      ...
      inversion_clear H3.
      case (Xmerge B1 B'1); simpl; try rewrite H2; auto;
        intros; try case o1, o2; try induction p; try induction p0;
          try case (collapse x); try case (collapse x0);
...
Lemma Xmerge_idempotent : forall (B:XBehaviour Sig),
  collapse B <> XUndefined -> Xmerge B B = B.
Proof.
...
Lemma Xmerge_inv_inject : forall (B1 B2:XBehaviour Sig) B,
  Xmerge B1 B2 = inject B -> exists B', B1 = inject B'.
Proof.
...
+ eq_dec_inj_elim b b0 Hbb0 H. clear Hbb0.
  clear IHB2_1 IHB2_2.
  elim (collapse_char' _ (Xmerge B1_1 B2_1)); intros.
  elim (collapse_char' _ (Xmerge B1_2 B2_2)); intros.
...
Lemma Xmerge_inv_inject' : forall (B1 B2:XBehaviour Sig) B,
  Xmerge B1 B2 = inject B -> exists B', B2 = inject B'.
Proof.
intros.
...
Lemma Xmerge_inv : forall (B1 B2:XBehaviour Sig) B,
  Xmerge B1 B2 = inject B -> exists B'1 B'2,
    B1 = inject B'1 /\ B2 = inject B'2 /\ merge B'1 B'2 = inject B.
Proof.
intros.
...
Definition Xmore_branches (XB XB':XBehaviour Sig) :=
  exists B B', XB = inject B /\ XB' = inject B' /\ more_branches B B'.
...
Lemma Xmore_branches_merge_extend : forall B1 B2 B1' B2' B,
  ...
  Xmerge B1 B2 = inject B -> Xmore_branches (Xmerge B1 B2) (Xmerge B1' B2').
Proof.
